...
class AdController extends Controller
{
    ...
    const LIVE_BANNER = "live_banner";
    const DEFAULT_JUMP_TYPE = "common_web";
    ...
    public function getAdListOfCompatible(UcManager $ucManager, ContentGuardContract $contentGuardContract)
    {
        ...
        if (self::isIOSOS($this->agent->getUserAgent())) {
    ...
    public function getAdList(UcManager $ucManager, ContentGuardContract $contentGuardContract, $locationCode)
    {
        ...
        try {
            ...
            if (self::IOS_TERMINAL == $terminalCode || self::ANDROID_TERMINAL == $terminalCode) {
                $appVersion = self::getAPPVersion($this->agent->getUserAgent());
                if (version_compare($appVersion, '2.5.*') < 0) {
                    $adsData = $this->adManager->getAdsData($locationCode, array_get($adIdList,'data.ad_access_id_list'), $terminalCode);
                } else {
            ...
            $resData = array_get($adsData,'data');

            if (self::LIVE_BANNER == $locationCode) {
                $appVersion = self::getAPPVersion($this->agent->getUserAgent());
                if ( empty($appVersion) ) {
                    $ret['code'] = APP_VERSION_NOT_FOUND;
                    $ret['data'] = 'get app version not found';
                    return $ret;
                }
                if ($this->agent->isAndroidOS()) {
                    if (version_compare($appVersion, '2.3.250') < 0) {
                        foreach ($resData as &$item) {
                            $item['jump_type'] = 'common_web';
                            unset($item['jump_params']);
                        }
                    }
                }
                if (self::isIOSOS($this->agent->getUserAgent())) {
                    if (version_compare($appVersion, '2.3.239') <= 0) {
                        foreach ($resData as &$item) {
                            $item['jump_type'] = 'common_web';
                            unset($item['jump_params']);
                        }
        ...
        try {
            ...
            if (self::IOS_TERMINAL == $terminalCode || self::ANDROID_TERMINAL == $terminalCode) {
                ...
                if (version_compare($appVersion, '2.5.*') < 0) {
                    ...
                    $adsData = $this->adManager->getAdAccessIdDatasBylocationCodes($locationCodes, array_get($adIdList,'data.ad_access_id_list'), $terminalCode, 0);
                }
            } else {
                $adsData = $this->adManager->getAdAccessIdDatasBylocationCodes($locationCodes, array_get($adIdList,'data.ad_access_id_list'), $terminalCode);
            }
            ...
            if ($liveBannerIndex >= 0) {
                ...
                if ($this->agent->isAndroidOS()) {
                    if (version_compare($appVersion, '2.3.250') < 0) {
                        ...
                        foreach ($resData[$liveBannerIndex] as &$item) {
                            $item['jump_type'] = 'common_web';
                            unset($item['jump_params']);
                        }
                    }
    ...
    public function getAdListBySpecialLocationCodes(UcManager $ucManager, ContentGuardContract $contentGuardContract)
    {
        $sessionId = $this->request->header('X-SessionId');
        if (empty($sessionId)) {
            abort(401);
        ...
        $reqData = $this->request->validate([
            'locationcode' => 'required|string',
            'expires_time' => 'nullable|integer'
        ]);

        $locationCodeStr = array_get($reqData, 'locationcode');
        if (null == array_get($reqData, 'expires_time')) {
            $expiresTime = time();
        } else {
            $expiresTime = array_get($reqData, 'expires_time');
        }
        ...
        try {
            ...
            $adsData = $this->adManager->getAdListBySpecialLocationCodes($locationCodes, array_get($adIdList, 'data.ad_access_id_list'), $terminalCode, $expiresTime, 0);
            ...
            $ret['code'] = SYS_STATUS_OK;
            $ret['errors'] = null;
            $ret['expires_time'] = intval(array_get($reqData, 'expires_time')) < time() ? time() + 86400 : array_get($reqData, 'expires_time');
        } catch (MatrixException $e) {
            Log::error("获取广告列表失败: {$e->getMessage()}", [$e]);
            $ret = [
                'code' => $e->getCode(),
     ...
     * 判断 是否是 ios 终端
     */
    private function isIOSOS(string $userAgent)
    {
        $result = false;
        if (strpos(strtolower($userAgent), 'iphone') || strpos(strtolower($userAgent), 'ios') || strpos(strtolower($userAgent), 'simulator')) {
            $result = true;
        }
        return $result;
    }
